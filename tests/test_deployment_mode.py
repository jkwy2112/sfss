...
class DeploymentModeTestBase(unittest.TestCase):
    ...
    def issue_service_token(self, permission):
        ...
        role = {"inbound_upload": "uploader", "inbound_download": "downloader",
                "outbound_upload": "red_uploader", "outbound_download": "green_downloader"}[permission]
        ...
        try:
            self.service.add_member("p1", "svc", role, "admin")
        except ServiceError:
            pass  # a cross-workflow role cannot exist here; deployment scope trips first
        request = json.dumps({"label": "agent", "username": "svc", "project_id": "p1",
...
class InboundSystemTest(DeploymentModeTestBase):
    ...
    def setUp(self):
        super().setUp()
        self.service.add_member("p1", "alice", "uploader", "admin")
        self.service.add_member("p1", "reader", "downloader", "admin")
    ...
    def test_green_upload_and_red_download_flow_is_served(self):
        ...
        status, _, downloaded = self.request(
            "GET", f"/v1/projects/p1/objects/{object_id}/download",
            headers=self.login_headers("r", "red"))
        self.assertEqual(200, status)
    ...
    def test_multipart_inbound_sessions_are_served(self):
        ...
        status, _, payload = self.request("POST", "/v1/projects/p1/uploads", request, {
    ...
    def test_outbound_routes_are_not_deployed(self):
        checks = (
            ("GET", "/v1/projects/p1/outbound", self.login_headers("a", "red")),
            ("GET", "/v1/projects/p1/outbound-policy", self.login_headers("a", "red")),
            ("POST", "/v1/projects/p1/outbound", {"Authorization": "Bearer a", "X-SFSS-Zone": "red",
    ...
    def test_outbound_roles_and_token_scopes_are_rejected(self):
        with self.assertRaisesRegex(ServiceError, "does not belong to this deployment"):
            self.service.add_member("p1", "reader", "green_downloader", "admin")
        self.assertEqual(set(), self.service.outbound_roles("p1", "alice"))
    ...
    def test_network_policy_cannot_grant_outbound_direction(self):
        policy = self.service.set_network_policy(
            "p1", {"inbound_upload_cidrs": ["127.0.0.1/32"], "outbound_upload_cidrs": ["10.9.0.0/16"]},
            "admin")
        ...
        with self.assertRaisesRegex(ServiceError, "not deployed"):
            self.service.require_source_ip("p1", "outbound", "127.0.0.1")
...
class OutboundSystemTest(DeploymentModeTestBase):
    ...
    def setUp(self):
        super().setUp()
        self.service.add_member("p1", "alice", "red_uploader", "admin")
        self.service.add_member("p1", "reader", "green_downloader", "admin")
        self.service.add_member("p1", "admin", "approver", "admin")
        self.service.set_outbound_policy("p1", {"enabled": True, "approval_provider": "local",
    ...
    def test_red_upload_approval_and_green_download_flow_is_served(self):
        ...
        status, _, payload = self.request("POST", "/v1/projects/p1/outbound", body, {
        ...
        status, _, payload = self.request(
            "POST", f"/v1/projects/p1/outbound/{transfer['id']}/decision", decision, {
                "Authorization": f"Bearer {self.admin_token()}", "Content-Type": "application/json",
        ...
        status, _, downloaded = self.request(
            "GET", f"/v1/projects/p1/outbound/{transfer['id']}/download",
    ...
    def test_inbound_routes_are_not_deployed(self):
        checks = (
            ("POST", "/v1/projects/p1/objects", {"Authorization": "Bearer a", "X-SFSS-Zone": "green",
                                                 "X-Filename": "netlist.txt", "Content-Length": "1"}),
            ("GET", "/v1/projects/p1/objects", self.login_headers("a", "green")),
            ("GET", "/v1/projects/p1/objects/o1/download", self.login_headers("r", "red")),
    ...
    def test_inbound_roles_and_token_scopes_are_rejected(self):
        with self.assertRaisesRegex(ServiceError, "does not belong to this deployment"):
            self.service.add_member("p1", "alice", "uploader", "admin")
        with self.assertRaisesRegex(ServiceError, "does not belong to this deployment"):
            self.service.add_member("p1", "reader", "downloader", "admin")
...
class DeploymentDatabaseScopeTest(unittest.TestCase):
    def test_inbound_runtime_rejects_outbound_records(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            store.execute("INSERT INTO projects(id,name,created_at) VALUES('p1','P1',1)")
            store.execute("INSERT INTO outbound_policies(project_id,enabled,approval_provider,"
                          "updated_at,updated_by) VALUES('p1',0,'local',1,'admin')")
    ...
    def test_outbound_runtime_rejects_inbound_records(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            store.execute("INSERT INTO memberships(project_id,username,role) "
                          "VALUES('p1','alice','uploader')")
            with self.assertRaisesRegex(ValueError, "deployment database scope violation"):
                create_runtime(settings)
